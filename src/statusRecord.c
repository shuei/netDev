...

static long init_record(pst,pass)
    ...
    int pass;
{
    ...
    if( pdset->init_record ) {
        if((status=(*pdset->init_record)(pst))) return(status);
    }
    return(0);
}

static long process(pst)
	struct statusRecord	*pst;
{
    ...
    long	  nNew;
{
    ...
    pst->nord = nNew;
    if(pst->nord > pst->nelm) pst->nord = pst->nelm;
    return(0);
}
    ...
    struct statusRecord	*pst;
{
	...
	monitor_mask |= (DBE_LOG|DBE_VALUE);
        if(monitor_mask) db_post_events(pst,&pst->ch01,monitor_mask);
	return;
